...
#include "imgui_dx8/imgui_impl_dx8.h"
#include <imgui.h>
#include "CLIData.h"
#include <fstream>
#include "Console.h"
...
extern IMGUI_API LRESULT ImGui_ImplDX8_WndProcHandler(HWND, UINT, WPARAM, LPARAM);
LRESULT STDMETHODCALLTYPE user_wndproc(HWND window, UINT message_type, WPARAM w_param, LPARAM l_param) {
    if (message_type == WM_KEYDOWN && w_param == VK_OEM_3)
    {
        if ((HIWORD(l_param) & KF_REPEAT) == 0) 
            disp = !disp;

        return true;
    }
    else if ((message_type == WM_KEYDOWN && w_param == VK_OEM_3) ||
             (message_type == WM_CHAR && w_param == 0x60))
    {
        return true;
    }

    if(disp)
        ImGui_ImplDX8_WndProcHandler(window, message_type, w_param, l_param);
...
HRESULT Direct3DDevice8Wrapper::EndScene() {
    ...
    if (!is_initialised) {
        ...
        if (game_hwnd != NULL) {
            ...
            ImGui_ImplDX8_Init(game_hwnd, this);
            // ..we're all done!
            is_initialised = true;
        }
    }
